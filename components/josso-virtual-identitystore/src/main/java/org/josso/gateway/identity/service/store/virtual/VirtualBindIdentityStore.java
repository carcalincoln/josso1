...
/**
 * Virtual Bindable Identity Store which builds on multiple identity data
 * sources for looking up user and entitlement records. It enables providing a
 * unified view of one or more identity silos for consumption by Single Sign-On
 * services. It is useful in scenarios with multiple idenitity silos,
 * potentially based on multiple technologies and information models, for which
 * an authoriative source for user information cannot be created or significant
 * effort is required.
 * 
...
public class VirtualBindIdentityStore extends VirtualIdentityStore implements BindableCredentialStore {
	...
	for (Iterator<IdentitySource> identitySourceIterator = getIdentitySources().iterator(); identitySourceIterator
		.hasNext();) {
		...
		try {
		    boolean bindResult = ((BindableCredentialStore) identitySource.getBackingIdentityStore())
	...
	selectedBindOutcomes = getIdentityDataMappingPolicy()
		.selectBindOutcomes(Collections.unmodifiableCollection(sourceBindOutcomes));
	if (selectedBindOutcomes != null && selectedBindOutcomes.size() > 0) {
	    // Use the configured mapping policy to create a joint result from selected
	    // entries
