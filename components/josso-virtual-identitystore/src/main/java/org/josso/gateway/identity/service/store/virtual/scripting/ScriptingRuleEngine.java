...
public class ScriptingRuleEngine {
    ...
    public ScriptingRuleExecutionOutcome execute(String ruleScriptName,
	...
	for (Iterator<ScriptingRuleParameter> scriptingRuleParameterIterator = ruleParameters
		.iterator(); scriptingRuleParameterIterator.hasNext();) {
	    ScriptingRuleParameter scriptingRuleParameter = scriptingRuleParameterIterator.next();
	    bsfManager.declareBean(scriptingRuleParameter.getName(), scriptingRuleParameter.getValue(),
		    scriptingRuleParameter.getType());
	}
	bsfManager.exec(script.lang, script.file.getCanonicalPath(), 0, 0, script.string);
	return outcome;
    }
     ...
     *
     * @param url     The script url consisting of a path and optional parameters
