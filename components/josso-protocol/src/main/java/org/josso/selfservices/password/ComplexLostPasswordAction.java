...
public class ComplexLostPasswordAction extends SelfServicesBaseAction {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
        	...
        	ProcessResponse pr = pwdService.startProcess(ctx.getSecurityDomain().getName()+"-complex-lostpassword");
                ...
                LostPasswordUrlProvider lostPaswordUrlProvider = new LostPasswordUrlProvider() {
                    public String provideResetUrl(String passwordAssertionId) {
                        return baseUrl + "&RPBAExterno=1&" + PARAM_JOSSO_PROCESS_ID + "=" + pid + "&" + "josso_" + Constants.CHALLENGE_PWD_ASSERTION_ID + "=" + passwordAssertionId;
                    }
                };
            ...
            log.error("Error recovering password RPBA : " + e.getMessage(), e);
            //request.setAttribute("error", e);

            if (e.getMessage().endsWith("Multiple users found")) {
