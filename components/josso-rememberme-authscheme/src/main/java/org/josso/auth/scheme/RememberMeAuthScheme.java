...
/**
 * This authentication scheme uses symetric encrypton to compare a received
 * token with a username stored in the configured identity store..
 *
 * Subclasses may provide different mechanisms to store / retrieve tokens
 * instead of enrcyption
 *
 * Created by IntelliJ IDEA. User: sgonzalez Date: Nov 10, 2008 Time: 2:36:29 PM
 * 
...
public class RememberMeAuthScheme extends AbstractAuthenticationScheme {
	...
	try {
		...
		throw new IOException(
			"Cannot find resource /josso-auth.properties.  Make sure this file is installed with JOSSO Gateway!");
	    authProps.load(is);
		...
		logger.warn(
			"Please, replace josso-auth.properties key! Do not use the one provided with the Gateway Archetype!");
	} catch (IOException e) {
	...
	if (remembermeToken == null || remembermeToken.length() == 0) {
	    if (logger.isDebugEnabled()) {
		logger.debug("RememberMe Token"
			+ (remembermeToken == null || remembermeToken.length() == 0 ? " not" : "") + " provided. ");
	    }
	...
	if (username == null || username.length() == 0) {
	    if (logger.isDebugEnabled()) {
		logger.debug("Username not " + (remembermeToken == null || remembermeToken.length() == 0 ? " not" : "")
			+ " provided. ");
	    }
	    // We don't support empty values !
	    return false;
	...
	if (!validateUsername(username, knownUsername)
		|| !validateRememberMeToken(remembermeToken, knownRemembermeToken)) {
	    return false;
	}
	if (logger.isDebugEnabled())
    ...
    /**
     * This implementation will retrieve some credentials from the identity store
     * but will also use input some credentials as known credentials
     *
	...
	// Add a new input credential with the username associated to the remember me
	// token
	...
	return username != null && knownUsername != null && username.length() > 0 && knownUsername.length() > 0
		&& username.equals(knownUsername);
    }
